                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(DB.ConnectString))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand())
                        {
                            ...
                            cmd.Parameters.AddWithValue("@MaHD", _MaHD);
                            cmd.Parameters.AddWithValue("@NgayVao", dateNgayVao.Value);
                            cmd.Parameters.AddWithValue("@MaKH", cbKH.SelectedValue ?? (object)DBNull.Value); // Handle potential null value
                            cmd.Parameters.AddWithValue("@MaNV", cbNV.SelectedValue ?? (object)DBNull.Value); // Handle potential null value
                            cmd.Parameters.AddWithValue("@PhuThu", string.IsNullOrEmpty(txtPhuThu.Text) ? (object)DBNull.Value : txtPhuThu.Text);
                            cmd.Parameters.AddWithValue("@PhuThuTheoPhanTram", cbPhuThu.Text == "Phần trăm (%)");
                            cmd.Parameters.AddWithValue("@GiamGia", string.IsNullOrEmpty(txtGiamGia.Text) ? (object)DBNull.Value : txtGiamGia.Text);
                            cmd.Parameters.AddWithValue("@GiamGiaTheoPhanTram", cbGiamGia.Text == "Phần trăm (%)");
                            cmd.Parameters.AddWithValue("@NgayThanhToan", DateTime.Now);
                            cmd.Parameters.AddWithValue("@ThanhTien", string.IsNullOrEmpty(txtThanhTien.Text) ? (object)DBNull.Value : txtThanhTien.Text);
                            cmd.Parameters.AddWithValue("@DaThanhToan", 1);
                            cmd.Parameters.AddWithValue("@MaBan", MaBan ?? (object)DBNull.Value); // Handle potential null value
